...
-   $T=f(X_1, X_2, \dots, X_n)$ é uma função da amostra que vai fornecer informação sobre $\theta$.
:::

:::{.callout-note}
## Ideia da distribuição amostral

:::columns
::::column

![](pop_amostra.png){width=75%}
::::
:::: column
...
* Ao gerar uma amostra de tamanho $n=1,2,3, \dots, 1000$ e calcular a proporção amostral em cada uma delas tem-se o seguinte resultado:

```{r, echo=FALSE}
library(ggplot2)

# Criando os valores de n
data <- data.frame(
  n = seq(1, 1000),
  p = sapply(seq(1, 1000), function(n) mean(rbinom(n, size = 1, prob = 0.5)))
)
...
* Ao gerar um $100$ amostras de tamanho $n=1,2,3, \dots, 1000$ e calcular a média amostral em cada uma delas, supondo $X\sim N(10,4)$, tem-se o seguinte resultado:


```{r, echo=FALSE, fig.align='center', fig.height=8, fig.width=14, message=FALSE, warning=FALSE}
n <- c(1, 5, seq(10,90, by=10), seq(100,1000, by=100)) 
n.sim <- 100
...
for(i in 1:length(n)){
  for(j in 1:n.sim){
    ...
    med    <- c(med, mean(rnorm(n[i],mean = 10, 2)))
    n.size <- c(n.size, n[i]) 
  }
}

dados <- data.frame(med, n.size)

#library(plotly)
library(ggplot2)

p <- ggplot(dados)+geom_jitter(aes(x =factor(n.size), y=med), width=0.1, alpha=0.25)+ 
  geom_hline(yintercept = 10, color="red")+xlab("Tamanho amostral")+ 
  ylab("Média amostral") + labs(title = "Exemplo TCL - X ~ N(10,4)")+
...
$$\sum_{i=1}^{n}(Z_{i}-\overline{Z})^2=\sum_{i=1}^{n}\dfrac{(X_{i}-\overline{X})^2}{\sigma^2}\sim \chi_{n-1}^{2}$$
...
**Observação:** Como $S^2$ é uma função linear de $U$ no corolário acima, a densidade de $S^2$ pode ser obtida a partir da densidade de $U$. $U\sim \chi_{n-1}^{2}$ e $S^2$ é dada por uma função monótona de $U$.

$$S^2=\dfrac{\sigma^2}{n-1}U,\ U=\dfrac{(n-1)}{\sigma^2}S^{2}\quad \text{e} \quad\dfrac{dU}{dS^2}=\dfrac{n-1}{\sigma^2}$$ 
...
$$\begin{array}{ll}
=& \displaystyle\int\limits_0^\infty f_{X,Y}(x, y) \, dy = \frac{1}{\Gamma\left(\frac{m}{2}\right) \Gamma\left(\frac{n}{2}\right) 2^{\frac{m+n}{2}}} \left(\frac{m}{n}\right)^{\frac{m}{2}} x^{\frac{m-2}{2}}\\
\times& \displaystyle\int\limits_0^\infty y^{\frac{m+n-2}{2}} \exp\left\{-\frac{1}{2} y \left(\frac{m}{n}x + 1\right)\right\} \, dy\\
...
\times& \displaystyle\int\limits_0^\infty \dfrac{\left[\frac{1}{2} \left(\frac{m}{n}x + 1\right)\right]^{\frac{m+n}{2}}}{\Gamma\left(\frac{m+n}{2}\right)} y^{\frac{m+n}{2} - 1} \exp\left\{-y \frac{1}{2} \left(\frac{m}{n}x + 1\right)\right\} \, dy
\end{array}
$$
...
<!-- Fecharemos esta subseção com algumas observações sobre a distribuição $F$. -->

:::{.callout-note}
## Observação 1:

* Se $X \sim F_{m, n}$, então $E(X) = \frac{n}{n - 2} \quad \text{para} \quad n > 2$

e 

$$\text{Var}(X) = \frac{2n^2 (m + n - 2)}{m (n - 2)^2 (n - 4)} \quad \text{para} \quad n > 4.$$
:::

:::{.callout-note}
## Observação 2:

* Se $X \sim F_{m, n}$, então $\frac{1}{X} \sim F_{n, m}$.

* Este resultado permite que a distribuição $F$ seja tabelada usando apenas a calda superior.

* Por exemplo, se o quantil $\xi_{0.95}$ é dado para uma $F_{m, n}$, o quantil $\xi^*_{0.05}$ para uma $F_{n, m}$ é dado por $\frac{1}{\xi_{0.95}}$.
...
&=& (2\pi)^{-1/2}\exp\left\{ -\dfrac{1}{2}z^{2} \right\}\dfrac{1}{\Gamma{(k/2)}}\\
&\times&\left(\dfrac{1}{2}\right)^{k/2}u^{\frac{k}{2}-1}\exp\left\{ -\dfrac{1}{2}u \right\}I_{(-\infty,\infty)}(v)I_{(0,\infty)}(u)
\end{array}
$$
...
$$X=\dfrac{Z}{\sqrt{U/k}},\quad Y=U\quad \text{e}\quad Z=X\sqrt{Y/k}.$$
...
Se $X$ é uma v.a. com f.d. igual a anterior, então dizemos que $X$ segue a distribuição t-Student com $k$-graus de liberdade.
:::
...
Se $Z\sim N(0,1)$ e $U\sim\chi_{k}^{2}$ são v.a's independentes, então 
...
Considere $X_{1}, X_{2}, \dots, X_{n}$ uma a.a. da $N(\mu, \sigma^2)$, então $Z=(\overline{X}-\mu)/(\sigma/\sqrt{n})\sim N(0,1)$  e $\dfrac{1}{\sigma^2}\sum_{i=1}^{n}(X_{i}-\overline{X})^2\sim \chi_{n-1}^{2}$.
:::
...
* Conforme o grau de liberdade cresce, a distribuição t-Student se aproxima da distribuição $N(0,1)$.

* O quadrado de uma variável aleatória com distribuição $t_k$ terá distribuição $F_{1,k}$ . Isto é, se $Y\sim t_k$ então $Y^{2}\sim F_{1, k}$ .
