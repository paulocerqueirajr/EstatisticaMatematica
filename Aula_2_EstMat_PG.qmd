...
L(\theta;\textbf{y})&=&c \ \prod\limits_{i=1}^{n}\dfrac{1}{\theta}I_{(0,\theta)}(x_{i})\\
 &=&\dfrac{c}{\theta^n}I_{(x_{(n)},\infty)}(\theta)
\end{array}
$$
...
L(\theta;\textbf{y})&=& \dfrac{1}{\theta^n}\prod\limits_{i=1}^{n}I_{(\theta,2\theta)}(y_{i})\\
 &=&\dfrac{1}{\theta^n}I_{(y_{(n)}/2,y_{(1)})}(\theta)
\end{array}
$$
...
* A última expressão é um polinômio de grau $n$ em $r$, onde o coeficiente $r^t$ sera $g(t){n\choose t}$. 
...
$\underbrace{(2\pi)^{-n/2}}_{h(x)}\underbrace{\theta^{-n}}_{c(\theta)}\exp\left\{ -\underbrace{\dfrac{1}{2\theta^2}}_{\omega_{1}(\theta)}  \underbrace{\sum\limits_{i=1}^{n}y_{i}^2}_{t_{1}(x)}+\underbrace{\dfrac{1}{\theta}}_{\omega_{2}(\theta)}\underbrace{\sum\limits_{i=1}^{n}y_{i}}_{t_{1}(x)}\right\}$
